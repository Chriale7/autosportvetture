...
function filterCars() {
    ...
    const prezzoMax = document.getElementById('filterPrezzo')?.value || '';
    
    filteredCars = allCars.filter(car => {
        const matchSearch = !searchText || 
            car.marca.toLowerCase().includes(searchText) || 
            car.modello.toLowerCase().includes(searchText);
        const matchTipo = !tipo || car.tipo === tipo;
        const matchCarburante = !carburante || car.carburante === carburante;
        const matchPrezzo = !prezzoMax || car.prezzo <= parseInt(prezzoMax);
        
        return matchSearch && matchTipo && matchCarburante && matchPrezzo;
    });
