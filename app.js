...
function displayCars(cars) {
    ...
    grid.innerHTML = cars.map(car => `
        <div class="car-card" onclick="window.location.href='dettaglio.html?id=${car.id}'" style="cursor: pointer;">
            <div class="car-image" style="background-image: url('${car.images[0] || ''}'); background-size: contain; background-repeat: no-repeat; background-position: center; height: 200px; background-color: #f3f4f6;"></div>
            <div class="car-info">
                ...
                <div class="car-actions" onclick="event.stopPropagation();">
                    <button class="btn btn-primary" onclick="contactAboutCar('${car.marca} ${car.modello}')"><img src="whatsapp-logo.jpg" alt="WhatsApp" style="width: 18px; height: 18px; vertical-align: middle; margin-right: 5px; border-radius: 3px;"> WhatsApp</button>
...
function updateCarsCount(count) {
    ...
    if (countElement && count !== undefined) {
        if (count === 0) {
            ...
            countElement.textContent = "Clicca su un'auto e scopri i dettagli";
        }
    }
}
