...
async function previewImages(event) {
    ...
    if (files.length > 50) {
        showNotification('Massimo 50 foto! Seleziona meno immagini.', 'error');
        event.target.value = '';
        return;
    }
    
    showNotification('Caricamento foto in corso...', 'info');
        ...
        try {
            ...
            if (file.type === 'image/heic' || file.type === 'image/heif' || file.name.toLowerCase().endsWith('.heic') || file.name.toLowerCase().endsWith('.heif')) {
                ...
                if (typeof heic2any === 'undefined') {
                    showNotification('Conversione HEIC non disponibile. Usa JPG o PNG.', 'error');
                    continue;
                }
            ...
            showNotification('Errore nel caricamento di alcune foto. Riprova.', 'error');
        }
    }
    
    // Clear the input so you can re-upload the same files if needed
    setTimeout(() => {
        event.target.value = '';
    }, 100);
}
